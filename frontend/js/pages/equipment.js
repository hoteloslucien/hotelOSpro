/* Hotel OS — Equipment V2 — FIXES COMPLETS v14.1 */
const EquipmentPage = {
  ...
  _draw: function() {
    ...
    if (visible.length > 0) {
      for (var idx = 0; idx < visible.length; idx++) {
        ...
        list += '<div class="inv-card" data-eq-open="' + item.id + '">' +
          '<div class="inv-card-header"><div class="inv-card-icon">' + (statusIcon[item.status] || '🔧') + '</div>' +
          '<div class="inv-card-title-wrap"><div class="inv-card-title">' + self._esc(item.name) + '</div>' +
          '<div class="inv-card-meta">' + (fam ? self._esc(fam.name) : '—') + ' · ' + (tp ? self._esc(tp.name) : '—') + '</div>' +
          '</div></div>' +
          '<div class="inv-card-body"><div class="inv-card-badges">' +
    ...
    document.getElementById('page-content').innerHTML =
      '<div class="page-header"><div><div class="page-h1">Équipements</div>' +
      '<div class="page-sub">' + this.items.length + ' au total · ' + this.families.length + ' familles</div></div>' +
      '<button class="btn btn-primary btn-sm" id="eq-new-btn">+ Ajouter</button></div>' +
  ...
  newItem: function() {
    ...
    if (typeOpts.length === 0) typeOpts = [{ value: '', label: '— Sélectionnez une famille d\'abord —' }];
    ...
    setTimeout(function() {
      ...
      var typeSel = document.getElementById('mf-type_id');
      if (famSel && typeSel) {
        famSel.addEventListener('change', function() {
          ...
          typeSel.innerHTML = filtered.length
            ? filtered.map(function(t) { return '<option value="' + t.id + '">' + t.name + '</option>'; }).join('')
            : '<option value="">— Aucun type pour cette famille —</option>';
        });
      }
    }, 100);
  },
  ...
  openItem: function(id) {
    ...
    var body = '<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px">' +
      '<span class="badge badge-' + (item.status === 'ok' ? 'basse' : item.status === 'en_panne' ? 'urgente' : 'normale') + '">'
      + (statusIcon[item.status] || '') + ' ' + (sLabel[item.status] || item.status) + '</span>'
      ...
      + '</div>'
      // ── Action buttons DANS LE BODY (pas footer) pour éviter le bug getElementById ──
      + '<div style="display:flex;flex-direction:column;gap:8px;margin-top:18px">'
      + '<button class="btn btn-secondary btn-full" id="eq-action-status">🔄 Changer le statut</button>'
      + '<button class="btn btn-primary btn-full" id="eq-action-edit">✏️ Modifier</button>'
      + (item.status === 'en_panne' || item.status === 'hors_service'
          ? '<button class="btn btn-warning btn-full" id="eq-action-inv">🛠 Créer une intervention</button>' : '')
      + '<button class="btn btn-secondary btn-full" style="color:var(--danger)" id="eq-action-del">🗑 Supprimer</button>'
      + '</div>';

    Modal.open('Équipement #' + item.id, body,
      '<button class="btn btn-secondary btn-full" onclick="Modal.close()">Fermer</button>');

    // Bind directly — no setTimeout needed since buttons are in body not footer
    var bind = function() {
      var statusBtn = document.getElementById('eq-action-status');
      if (statusBtn) statusBtn.addEventListener('click', function() {
        Modal.close(true);
        setTimeout(function() {
          Modal.form('Changer le statut — ' + self._esc(item.name), [
      ...
      var editBtn = document.getElementById('eq-action-edit');
      if (editBtn) editBtn.addEventListener('click', function() {
        ...
        setTimeout(function() {
          ...
          Modal.form('Modifier — ' + self._esc(item.name), [
      ...
      var invBtn = document.getElementById('eq-action-inv');
      if (invBtn) invBtn.addEventListener('click', function() {
        Modal.close(true);
        setTimeout(function() {
          Modal.form('Créer une intervention', [
            { key: 'title',       label: 'Titre *',    value: 'Panne : ' + self._esc(item.name) },
            ...
            await Api.createIntervention({
              ...
              equipment_item_id: item.id, room_id: item.room_id || null,
            });
      ...
      var delBtn = document.getElementById('eq-action-del');
      if (delBtn) delBtn.addEventListener('click', function() {
        Modal.close(true);
        setTimeout(function() {
          Modal.confirm('Supprimer "' + self._esc(item.name) + '" ?', async function() {
            try {
    ...
    // Bind immediately (buttons are in modal body, rendered synchronously)
    bind();
  },
